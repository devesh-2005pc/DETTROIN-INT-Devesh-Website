    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-8 bg-slate-950/90 backdrop-blur-2xl">
        ...
        {onPrev && (
          <button
            onClick={onPrev}
            className="absolute left-6 top-1/2 -translate-y-1/2 p-3 rounded-full bg-white/10 hover:bg-white/20 text-white backdrop-blur-md transition-all z-20 hover:scale-110"
          >
        ...
        {onNext && (
          <button
            onClick={onNext}
            className="absolute right-6 top-1/2 -translate-y-1/2 p-3 rounded-full bg-white/10 hover:bg-white/20 text-white backdrop-blur-md transition-all z-20 hover:scale-110"
          >
        ...
        <motion.div
          ...
          className="relative max-w-5xl max-h-[80vh] z-10 rounded-2xl overflow-hidden shadow-2xl border border-white/10"
        >
          <img
            src={image.src || image.url || image}
            alt={image.title || 'Gallery Preview'}
            className="w-full h-full max-h-[80vh] object-contain bg-slate-900"
          />
          {image.category && (
